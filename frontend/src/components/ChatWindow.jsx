...
import { Paperclip, Mic, Send, X, FileText, Wand2, Plus, Globe, Image as ImageIcon } from 'lucide-react';
...
function ThinkingBar({ text }) {
  if (!text) return null;
  return (
    <div className="flex items-center gap-2.5 px-4 sm:px-6 mb-3 text-[13px] text-zinc-500 dark:text-zinc-400">
      <span className="w-3.5 h-3.5 border-2 border-zinc-300 dark:border-zinc-600 border-t-zinc-600 dark:border-t-zinc-300 rounded-full animate-spin shrink-0" />
...
function ImageGeneratingAnimation() {
  ...
  const steps = ['Prompt samajh raha hoon…', 'Image bana raha hoon…', 'Details add…', 'Almost ready…'];
  useEffect(() => {
...
const IMAGE_KEYWORDS = ['image', 'photo', 'picture', 'draw', 'generate image', 'create image', 'banao image', 'tasveer', 'wallpaper', 'poster', 'illustration', 'banao', 'bana do'];
const EDIT_KEYWORDS = ['edit', 'improve', 'enhance', 'better', 'fix', 'accha', 'acha', 'sudhar', 'badal', 'hd', 'quality', 'clear'];
const SEARCH_KEYWORDS = ['search', 'latest', 'news', 'today', 'current', 'price', 'weather', 'score', '2025', '2026'];
...
export default function ChatWindow({ mode, sessionId, messages, setMessages, isGuest }) {
  ...
  const toggleListening = () => {
    if (!recognitionRef.current) { alert('Voice: Chrome use karo'); return; }
  ...
  const generateImage = async (promptText, photo) => {
    ...
    try {
      ...
      const res = await fetch(
        (import.meta.env.VITE_API_URL || 'https://setrxai-backend.onrender.com') + '/api/generate-image',
        {
          method: 'POST',
          headers: {
      ...
      setMessages((prev) =>
        prev.concat([{ role: 'assistant', content: '⚠️ Image failed: ' + err.message }])
      );
    } finally {
      setImgLoading(false);
      setImgEditing(false);
  ...
  const sendMessage = async () => {
    ...
    const displayText =
      input ||
      (attachedFile ? '📄 "' + attachedFile.name + '"' : hasPhoto ? '📷 Photo' : '');
    const userMessage = {
      role: 'user',
      content: displayText,
    ...
    setThinkingText('Soch raha hoon…');
    ...
    try {
      ...
      setMessages((prev) => {
        const u = prev.slice();
        u[u.length - 1] = { role: 'assistant', content: '⚠️ Connection error' };
        return u;
      });
    } finally {
      setLoading(false);
  ...
  const placeholder = imagePreviews.length
    ? 'Photo attached — sawal poocho ya "accha banao"…'
    : forceImageGen
    ? 'Image describe karo…'
    : 'Message likho…';
  ...
  return (
    <div className="flex flex-col h-full min-h-0 bg-zinc-50 dark:bg-[#0c0c0f]">
      ...
      <div ref={scrollContainerRef} className="flex-1 min-h-0 overflow-y-auto">
        <div className="max-w-3xl mx-auto w-full px-3 sm:px-4 py-6">
          {messages.length === 0 && <ModeHero mode={mode} onSelect={(t) => setInput(t)} />}
              ...
              return (
                <div key={i} className="mb-6">
                  <div className="flex items-center gap-2 mb-2">
                    <div className="w-6 h-6 rounded-full bg-zinc-200 dark:bg-zinc-800 flex items-center justify-center text-[10px]">✨</div>
                  ...
                  <div className="flex gap-3 mt-2 text-xs text-zinc-500">
                    <a href={imgUrl} target="_blank" rel="noreferrer" className="hover:text-zinc-800 dark:hover:text-zinc-200">
                      Download
                    </a>
                    <button type="button" className="hover:text-zinc-800 dark:hover:text-zinc-200" onClick={() => useImageForEdit(imgUrl)}>
                      Edit
                    </button>
                    <button
                      type="button"
          ...
          {(thinkingText || (waitingFirstChunk && !isSearching)) && (
            <ThinkingBar text={thinkingText || 'Soch raha hoon…'} />
          )}
          {isSearching && <ThinkingBar text="Web search…" />}
          {imgLoading && <ImageGeneratingAnimation />}
        </div>
      </div>

      {/* Input — Claude-like clean bar */}
      <div className="border-t border-zinc-200/80 dark:border-white/[0.06] bg-white/80 dark:bg-[#0c0c0f]/90 backdrop-blur-md">
        <div className="max-w-3xl mx-auto px-3 sm:px-4 py-3">
          {imagePreviews.length > 0 && (
            <div className="flex flex-wrap gap-2 mb-2">
              {imagePreviews.map((p, idx) => (
                <div key={idx} className="relative">
                  <img src={p.previewUrl} alt="" className="h-14 w-14 object-cover rounded-lg border border-zinc-200 dark:border-white/10" />
                  <button
                    type="button"
          ...
          <div className="flex items-end gap-1 rounded-2xl border border-zinc-200 dark:border-white/10 bg-zinc-50 dark:bg-white/[0.04] px-1.5 py-1.5 shadow-sm focus-within:border-zinc-400 dark:focus-within:border-white/20 transition-colors">
            ...
            <div className="relative" ref={plusMenuRef}>
              <button
                ...
                className="p-2.5 rounded-xl text-zinc-500 hover:bg-zinc-200/60 dark:hover:bg-white/10 transition"
              >
              ...
              {showPlusMenu && (
                <div className="absolute bottom-12 left-0 z-50 min-w-[180px] rounded-xl border border-zinc-200 dark:border-white/10 bg-white dark:bg-zinc-900 shadow-xl p-1">
                  <button type="button" onClick={() => fileInputRef.current && fileInputRef.current.click()} className="flex w-full items-center gap-2 px-3 py-2.5 rounded-lg text-sm hover:bg-zinc-100 dark:hover:bg-white/5">
                    <Paperclip size={15} /> Photos (max 4)
                  </button>
                  <button type="button" onClick={() => docInputRef.current && docInputRef.current.click()} className="flex w-full items-center gap-2 px-3 py-2.5 rounded-lg text-sm hover:bg-zinc-100 dark:hover:bg-white/5">
            ...
            <button
              ...
              className={'p-2.5 rounded-xl transition ' + (isListening ? 'text-red-500 bg-red-50 dark:bg-red-500/10' : 'text-zinc-500 hover:bg-zinc-200/60 dark:hover:bg-white/10')}
            >
              <Mic size={20} />
            </button>
          ...
          <p className="text-[10px] text-center text-zinc-400 mt-2">SetrxAI can make mistakes. Check important info.</p>
        </div>
      </div>
    </div>
  );
