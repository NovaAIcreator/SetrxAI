...
function TypingDots() {
  ...
  return (
    <div className="flex gap-2 sm:gap-3 justify-start mb-4 px-2 sm:px-4">
      ...
      <div className="flex items-center gap-2 rounded-2xl rounded-bl-md px-3.5 py-2 bg-white/5 dark:bg-white/[0.04] border border-white/10 backdrop-blur-sm">
        <div className="relative w-3.5 h-3.5 shrink-0">
          <div className="absolute inset-0 rounded-full border-2 border-purple-500/20" />
          <div className="absolute inset-0 rounded-full border-2 border-t-purple-400 animate-spin" />
        </div>
        <span className="text-sm text-zinc-400 truncate">{steps[step]}</span>
        <span className="flex gap-0.5 shrink-0">
          <span className="w-1 h-1 rounded-full bg-purple-400/80 animate-bounce" style={{ animationDelay: '0s' }} />
          <span className="w-1 h-1 rounded-full bg-purple-400/80 animate-bounce" style={{ animationDelay: '0.15s' }} />
          <span className="w-1 h-1 rounded-full bg-purple-400/80 animate-bounce" style={{ animationDelay: '0.3s' }} />
        </span>
      </div>
    </div>
  );
...
function ImageGeneratingAnimation() {
  ...
  return (
    <div className="flex gap-2 sm:gap-3 justify-start mb-4 px-2 sm:px-4">
      <div className="w-7 h-7 sm:w-8 sm:h-8 shrink-0 rounded-full bg-gradient-to-br from-purple-500 to-fuchsia-500 flex items-center justify-center text-xs animate-pulse">🎨</div>
      <div className="flex flex-col gap-3 bg-white/5 dark:bg-white/[0.04] border border-white/10 backdrop-blur-sm rounded-2xl rounded-bl-md px-4 py-3.5 min-w-[260px] max-w-xs">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-medium text-zinc-400">{steps[step]}{dots}</span>
        </div>
        <div className="relative h-1 bg-zinc-100/10 rounded-full overflow-hidden">
...
function WebSearchAnimation() {
  ...
  return (
    <div className="flex gap-2 sm:gap-3 justify-start mb-4 px-2 sm:px-4">
      ...
      <div className="flex items-center gap-2 rounded-2xl rounded-bl-md px-3.5 py-2 bg-white/5 dark:bg-white/[0.04] border border-white/10 backdrop-blur-sm">
        <Globe size={13} className="text-blue-400 animate-spin shrink-0" style={{ animationDuration: '1.8s' }} />
        <span className="text-sm text-zinc-400">{steps[step]}</span>
        <span className="flex gap-0.5">
          <span className="w-1 h-1 rounded-full bg-blue-400/80 animate-bounce" style={{ animationDelay: '0s' }} />
          <span className="w-1 h-1 rounded-full bg-blue-400/80 animate-bounce" style={{ animationDelay: '0.15s' }} />
          <span className="w-1 h-1 rounded-full bg-blue-400/80 animate-bounce" style={{ animationDelay: '0.3s' }} />
        </span>
      </div>
    </div>
  );
...
export default function ChatWindow({ mode, sessionId, messages, setMessages, isGuest }) {
  ...
  const generateImage = async (promptOverride) => {
    ...
    try {
      ...
      shouldScrollRef.current = true;
      // ★★★ YE LINE SABSE IMPORTANT HAI — IMAGE FIX ★★★
            ...
            return (
              <div key={i} className="flex gap-2 sm:gap-3 justify-start mb-4 px-2 sm:px-4">
                ...
                <div className="flex flex-col gap-2 max-w-sm">
                  ...
                  <a href={imgUrl} target="_blank" rel="noreferrer" download={`setrxai-${Date.now()}.jpg`} className="flex items-center gap-1.5 text-xs text-purple-500 hover:text-purple-400 px-1 transition-colors">
                    ⬇️ Download image
                  </a>
                </div>
              </div>
