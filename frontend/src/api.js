...
export const api = {
  ...
  getMessages: (sessionId) => apiRequest('/api/sessions/' + sessionId + '/messages').then((r) => r.json()),
  ...
  deleteProject: (id) => apiRequest('/api/projects/' + id, { method: 'DELETE' }).then((r) => r.json()),

  parseFile: (fileName, mimeType, base64Data) =>
    apiRequest('/api/parse-file', {
      method: 'POST',
      body: JSON.stringify({ fileName, mimeType, data: base64Data }),
    }).then((r) => r.json()),

  chatStream: (mode, messages, sessionId, image, file, images) => {
    const token = getToken();
    return fetch(API_URL + '/api/chat', {
      ...
      body: JSON.stringify({ mode, messages, sessionId, image, images, file }),
    });
  },
};
